...
class Solution
  ...
  def group_last_names
    @coll.find.aggregate([
      { '$group' => { _id: { age: '$group', gender: '$gender' }, last_names: { '$push' => '$last_name' } } }
    ])
  end

  def group_last_names_set
    @coll.find.aggregate([
      { '$group' => { _id: { age: '$group', gender: '$gender' }, last_names: { '$addToSet' => '$last_name' } } }
    ])
  end
  ...
  def age_groups_faster_than age_group, criteria_time
    @coll.find.aggregate([
      { '$match' => { secs: { '$lte' => criteria_time }, age_group: age_group } },
      { '$group' => { _id: { age: '$group', gender: '$gender' },
                      runners: { '$sum' => 1 },
                      fastest_time: { '$min' => '$secs' } } }
    ])
